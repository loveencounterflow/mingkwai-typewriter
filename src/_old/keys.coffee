...
XE.listen_to_all ( key, d ) -> whisper 'µ99823', 'ime', jr d
{ emit
  delegate
  listen_to_all
  listen_to
...
#-----------------------------------------------------------------------------------------------------------
@register = ( key, value, handler ) ->
  if ( @_get_key_handler { key, value, } )?
    throw new Error "facet #{rpr key} / #{rpr value} already registered"
    # continue if key is 'event'
  @_bindings.push [ key, value, handler, ]
  return null

#-----------------------------------------------------------------------------------------------------------
@_get_key_handler = ( uie ) ->
  for [ key, value, handler, ] in @_bindings
    if uie[ key ] is value then return handler
  return null
...
#
#-----------------------------------------------------------------------------------------------------------
listen_to 'KEYS/kblevels/change', @, ( { S, key, } ) ->
